...
1. **Trees & Flora (Decorators)**: Add a decorator pass during chunk generation
...
3. **3D Noise / Caves**: Introduce 3D noise to carve out cave networks below the
   surface.
4. **Creatures/Entities**: Add mobile, AI-driven entities (mobs/animals) that
...
6. **More Block Types**: Expand the block palette with new materials and
   properties to allow for richer building and terrain variation.
7. **Better underwater fog**: Model the colour shift that happens as a player
   gets deeper into water.
8. **Player Object Rendering**: Render a 3D model/mesh for the player character
   instead of just relying on the camera's perspective, allowing third-person
   views and visible avatars.
9. **Shadow Mapping**: Implement directional shadows cast by the sun and moon
   across the voxel terrain to add depth and realism to the lighting.
10. **Expand Configuration File**: We now have a `config.toml` that handles
